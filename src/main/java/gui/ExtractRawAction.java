...
public class ExtractRawAction implements ActionListener {


    private JFrame fram;
    private StoredConfig config;

    public ExtractRawAction(JFrame fram, StoredConfig config) {
        this.fram = fram;
        this.config = config;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (config.getValues().containsKey(Operations.EXTRACT_BMP)) {
            cfg = config.getValues().get(Operations.EXTRACT_BMP);
        }

        JFileChooser chooser = new JFileChooser(".");
        FileNameExtensionFilter filter = new FileNameExtensionFilter("RAW File", "raw");
        chooser.setFileFilter(filter);
        chooser.setSelectedFile(new File(cfg == null ? "title2b.raw" : cfg.getData().get(ConfigLineType.ORIGINAL_PATH)));
        chooser.setDialogTitle("Выбери графический  RAW файл");
        ...
        chooser.setDialogTitle("Выбери путь к BMP файлу");
        filter = new FileNameExtensionFilter("BMP File", "bmp");
        chooser.setFileFilter(filter);
        chooser.setSelectedFile(new File(cfg == null ? "title2b.bmp" : cfg.getData().get(ConfigLineType.NEW_PATH)));
        ...
        config.getValues().put(Operations.EXTRACT_BMP, cfg);
